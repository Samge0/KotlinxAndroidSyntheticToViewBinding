...
fun String.toCamelCase():  String {
    ...
    return split("_").mapIndexed { index, s ->
        if (index > 0) s.replaceFirstChar { it.uppercase() } else s.replaceFirstChar{ it.lowercase() }
    }.joinToString("")
}

fun String.toCapitalCase(): String = split("_").joinToString(" ") { it.replaceFirstChar { char -> char.uppercase() } }

fun String.toConstantCase(): String = uppercase().replace(" ", "_")

fun String.toDotCase(): String = lowercase().replace(" ", ".")

fun String.toHeaderCase(): String = split(" ").joinToString("-") { it.replaceFirstChar { char -> char.uppercase() } }

fun String.toNoCase(): String = lowercase().replace("_", " ")

fun String.toParamCase(): String = lowercase().replace(" ", "-")

fun String.toPascalCase(): String = split("_").joinToString("") { it.replaceFirstChar { char -> char.uppercase() } }
